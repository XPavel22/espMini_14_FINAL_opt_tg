...
void WiFiManager::loop() {
    ...
    if (isInFallbackAP) {
        if (millis() - apStartTime > apTimeout) {isInFallbackAP = false;
            connectToWiFi();
        }
        return;
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        if (isConnecting) {
            ...
            if (!settings.ws.networkSettings.empty()) {
                NetworkSetting& net = settings.ws.networkSettings[0];setIpAdresesInConfig(net);
...
void WiFiManager::connectToWiFi() {
    ...
    if (settings.ws.networkSettings.empty()) {isInFallbackAP = true;
        apStartTime = millis();
        startAccessPoint();
        return;
    }
    ...
    if (net.ssid.isEmpty()) {isInFallbackAP = true;
        apStartTime = millis();
        startAccessPoint();
        return;
    }
...
void WiFiManager::startAccessPoint() {appState.isStartWifi = true;

    setupAccessPointMode();

    IPAddress apIP = WiFi.softAPIP();if (!MDNS.begin(settings.ws.mDNS.c_str())) {} else {
        MDNS.addService("http", "tcp", 80);}
...
void WiFiManager::setupStationMode(const NetworkSetting& net) {
    ...
    if (net.useStaticIP) {WiFi.config(net.staticIP, net.staticGateway, net.staticSubnet, net.staticDNS);
    } else {}
}
...
void WiFiManager::setIpAdresesInConfig(NetworkSetting& net) {
    ...
    if (!net.useStaticIP) {net.staticIP = WiFi.localIP();
