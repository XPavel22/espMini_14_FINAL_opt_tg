...
void TelegramBot::begin() {
  ...
  if (myBot.begin()) {
    ...
    Serial.println("Requesting bot info (getMe)...");
    const char* botUser = myBot.getBotName();
    if (botUser && strlen(botUser) > 0) {
      this->settings.ws.telegramSettings.botUser = String(botUser);
      Serial.printf("✅ Bot username received and saved: %s\n", this->settings.ws.telegramSettings.botUser.c_str());
    } else {
      Serial.println("⚠️ Bot has no public username set. Link will not be available.");
    }
...
void TelegramBot::sendSimpleStatus(int64_t chatId) {
  ...
  offset += deviceManager.currentStateSensors(messageBuffer + offset, STATUS_BUFFER_SIZE - offset, false);
